...
```
string foo = "Hello";
string bar = "How are you?";int x = 5; 
...
// string converted to Upper case string upperstr2 = str2.ToUpper();  
...
// this will show the value 5 int index1 = str.IndexOf('s'); 
...
```
// Get values from this string.string value = "Dot Net Perls"; 
//variable first contains letter D char first = value[0]; 
//Second contains letter ochar second = value[1]; 
//last contains letter schar last = value[value.Length - 1];
```
...
// Call ToLower instance method, which returns a new copy.string lower = mixedCase.ToLower(); 
...
```
int id = 100 
// We can use an expression with a string interpolation.string multipliedNumber = $"The multiplied ID is {id * 10}."; 
